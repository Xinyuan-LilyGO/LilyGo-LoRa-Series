...
void loop()
{
  ...
  LT.writeBufferChar(trackerID, sizeof(trackerID));     //+4 bytes (3 characters plus null (0) at end)
  LT.writeFloat(latitude);                          //+4 = 8 bytes
  LT.writeFloat(longitude);                         //+4 = 12 bytes
  LT.writeUint16(altitude);                         //+2 = 14 bytes
  LT.writeUint8(satellites);                        //+1 = 15 bytes
  LT.writeUint16(voltage);                          //+2 = 17 bytes
  LT.writeInt8(temperature);                        //+1 = 18 bytes total to send
  TXPacketL = LT.endWriteSXBuffer();                //closes packet write and returns the length of the packet to send

  BytesSent = LT.transmitSXBufferIRQ(0, TXPacketL, 5000, TXpower, WAIT_TX);   //set a TX timeout of 5000mS
